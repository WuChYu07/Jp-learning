...
| ID  | 項目                | 狀態        | 說明                                                                        |
| --- | ------------------- | ----------- | --------------------------------------------------------------------------- |
| P1  | 手機可穩定學習      | in_progress | 冷啟動橫幅改為 sticky＋秒數；單字／文法庫慢載入提示；待 CORS＋手機再實測 |
| P2  | Notion 同步品質回報 | done        | 確認後回傳 SyncReport；Upload 顯示來源／新增／更新／略過／去重表（原 R5）    |
| P3  | 知識圖譜語意品質    | done        | 門檻 0.93；Map 信心過濾／修剪弱連線／批次重算；學習路徑不再自動 sync（原 R6） |
| P6  | 練習模式擴充        | done        | 複習卡發音補強；例句挖空四選一；AI 問答／帶提示翻譯＋`practice_dialogues`   |
...
| P6b | 配對翻牌遊戲 | deferred | Play Hub／配對翻牌暫擱置，優先練習模式                       |
...
2. **P4** — 備份匯出（有痛點再做）
3. 部署後於 Supabase 執行 `migrations/014_practice_dialogues.sql`（AI 練習紀錄表）
