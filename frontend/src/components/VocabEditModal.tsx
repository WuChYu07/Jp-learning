    ...
    >
      <form onSubmit={(e) => void handleSubmit(e)} className="space-y-4">
        ...
        <label className="block space-y-1">
          ...
          <input
            value={draft.reading}
            onChange={(e) => setDraft((p) => ({ ...p, reading: e.target.value }))}
            className="w-full rounded-xl border border-orange-100 bg-stone-50 px-3 py-2 text-sm outline-none focus:border-orange-300"
          />
        </label>
          ...
          {draft.example_sentences.map((ex, i) => (
            <div key={i} className="space-y-2 rounded-xl bg-stone-50 p-3 ring-1 ring-stone-100">
              <input
                ...
                placeholder="日文例句"
                className="w-full rounded-lg border border-stone-200 bg-white px-3 py-1.5 text-sm outline-none"
              />
              <input
                value={ex.chinese || ""}
                onChange={(e) =>
        ...
        <label className="block space-y-1">
          ...
          <textarea
            ...
            onChange={(e) => setDraft((p) => ({ ...p, notes_zh: e.target.value }))}
            rows={4}
